...
# Очистка истории
@AiEngineering.route("/clear", methods=["POST"])
def clear_history():
...
@AiEngineering.route("/gpt4free_query", methods=["POST"])
def gpt4free_query():
    ...
    try:
        ...
        # print(f"Error occurred with provider {g4f.Provider.GeekGpt.__name__}: {e}")
...
@AiEngineering.route("/")
@AiEngineering.route("/index")
def index():
    return render_template("index.html")

@AiEngineering.route("/documentation")
def documentation():
    return render_template("documentation.html")

if __name__ == "__main__":
    AiEngineering.run(debug=True)